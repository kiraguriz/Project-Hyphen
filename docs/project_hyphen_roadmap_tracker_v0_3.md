...
| HYP-M4-008 | `[x]` | P0 | Reliability | Notification storm test | HYP-M3-004 | No duplicate flood; UI remains responsive | Manual/automated test |
...
| M4 Beta Hardening | 3 | 0 | 5 | 4 |
| M5 Distribution | 3 | 0 | 2 | 5 |
...
- 2026-06-12 — HYP-M4-008 `[x]` — Added automated notification storm coverage on both sides of the mirror pipeline. Android `NotificationMirrorEventSender` now has a 1,000-event storm test proving only the first occurrence of each of 25 stable `sbnKey` values posts, repeats become updates, removals shrink active state, and every envelope stays acked under `notifications.v1`. macOS `NotificationMirrorReceiver` now has a matching 1,000-event coalescing test proving repeated Android keys overwrite the same macOS notification identifier and removals keep delivered notifications bounded. Verified: `./gradlew test assembleDebug` green; `swift test` green; `./scripts/check.sh` green.
