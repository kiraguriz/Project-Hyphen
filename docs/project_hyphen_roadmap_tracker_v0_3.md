...
| HYP-M2-011 | `[x]` | P0 | Pairing | Implement SAS confirmation UI both sides | HYP-M2-009,HYP-M2-010 | User confirms matching code before trust is stored | Manual test |
...
- 2026-06-11 — HYP-M3-008 `[~]` — Android→macOS text/link core contract slice landed, but runtime/manual acceptance remains open. Android: `TextLinkMessage` + `TextLinkSender` encode `text.send` under `text.v1`, require ack, cap payload values at 8192 chars, and reject non-http(s) URL schemes. macOS: new `HyphenText` target turns received `text.send/text.v1` envelopes into pending `TextLinkConfirmationRequest`s instead of opening/copying automatically. Protocol doc §7.1 now specifies the payload. Verified: `./gradlew test assembleDebug` green; `swift test` green. Remaining before `[x]`: wire Android send UI to a live `ProtocolSession`, wire macOS `TextLinkReceiver` to session callbacks and an NSAlert/user-confirmation action, then run the manual Android→Mac send test on a paired device/session.
