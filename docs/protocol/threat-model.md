...
| Hidden notification content still crosses the LAN | A1/A5 | Per-app privacy is enforced **source-side** when both peers negotiate `notifications.v1.privacyPolicy`: Android strips body/actions (`hideBody`) or all visible content (`existsOnly`) before serializing. Until Android applies the Mac-pushed policy, negotiated sessions fail closed with `existsOnly` at the source; after apply (or when the option is not negotiated), hidden content under the active mode never leaves the phone. The macOS receiver applies the same per-app rules locally as defense-in-depth for older/unsynced peers (protocol §7.1.1) | Designed |
