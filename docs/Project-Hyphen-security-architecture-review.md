...
> **2026-06-18 修复进展【来源：评估】**：接收端 progress/checkpoint/complete 已改为 O(1) 计数器（`receivedCount`/`highestContiguousIndex`）；`TransferResourceLimits` 限制并发、staging、registry、tombstone 与 outstanding；bounded tombstone 替代无限 `completed` map。发送前整文件 SHA-256 代价仍在；百万分块 hot path 线性扫描问题已在代码层关闭。
...
> **2026-06-18 修复进展【来源：评估】**：Android sender 区分 `sentChunks` 与 ACK 驱动的 `completedChunks`；receiver 区分 `receivedChunks` 与 contiguous checkpoint。`TransferCheckpointStore` 提供进程重启后的 peer-bound partial 恢复（单元已证；真机 kill/restart 仍 environment-only）。1 GiB / Doze / sleep-wake 仍无发布级证据。
...
- **状态（2026-06-18）**：**部分修复【来源：评估】**。Android 交付至 app external `received/`，macOS 交付至 Downloads（HYP-M3-016 `[~]`）。交互式 SAF/save panel 与真机打开/分享仍缺 `[?]` 实测。
- **位置**
...
- **状态（2026-06-18）**：**已修复（代码）【来源：评估】**。`TransferSender.cancel(fileId)` 关闭 stream、清理 registry/outstanding、记录 tombstone，并由同一实例发送 `transfer.cancel`；UI 已绑定 active sender。paired-session 手动验收仍 `[?]`（HYP-M3-014 `[~]`）。
- **位置**
...
- **状态（2026-06-18）**：**已修复（代码）【来源：评估】**。O(1) 接收进度、重复 chunk no-op、`TransferResourceLimits` 与 bounded tombstone；单元测试覆盖 quota 与 progress 分层（HYP-M3-018/019 `[x]`）。
- **位置**
...
- **状态（2026-06-18）**：**部分修复【来源：评估】**。`TransferCheckpointStore` 持久化 inbound/outbound（file-backed）checkpoint；trust revoke 清理 checkpoint + macOS `tokenStore.invalidatePeer`。Resume token 仍 in-memory + 10 分钟 TTL，无全局 purge timer（post-v0）。kill/restart E2E 仍 `[?]`。
- **位置**
...
- **状态（2026-06-18）**：**已修复（代码）【来源：评估】**。Sender/receiver progress 分层；bounded tombstone + `recycleSession`/`handleAckTimeout`；HYP-M3-019 `[x]`。
- **位置**
...
- **状态（2026-06-18）**：**未改代码；评估记录【来源：评估】**。保留每帧 `flush()`；未做 batching（缺 paired-device 吞吐证据，batching 可能伤害 ACK/heartbeat 小帧延迟）。见 dimension-02 开放问题。
